...
import com.Junglee.Utilities.ExcelReader;
import com.Junglee.Utilities.ExtentTestManager;
import com.Junglee.Utilities.GoogleSheet;
...
public class Provider{
	...
	@DataProvider
	public Object[][] insertRest(Method m)
	{
		System.out.println("Data Provider Executing method name:-"+m.getName());
		Class<? extends Object> className = m.getDeclaringClass();

		Package pack = m.getDeclaringClass().getPackage();
        String packageName = pack.getName().split(".Junglee.APITest.")[1];
		...
		System.out.println(System.getProperty("user.dir")+"/src/test/resources/excel/"+packageName+".xlsx");
			ExcelReader excel = new ExcelReader(System.getProperty("user.dir")+"/src/test/resources/excel/"+packageName+".xlsx");
		...
		if(!allData.containsKey(m.getName()))
		{
			...
			ExtentTestManager.getTest().assignCategory(className.getSimpleName());
			ExtentTestManager.getTest().log(LogStatus.INFO,"Test Going to be skipped",m.getName());
			throw new SkipException("Test Case Skipped as not present in Test Data XL File - Sheet :"+className.getSimpleName()+" Excel");
		}

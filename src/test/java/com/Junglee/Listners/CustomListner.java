...
import com.Junglee.Utilities.ScreenShot;
...
public class CustomListner implements ITestListener, IInvokedMethodListener {
	...
	public void onTestStart(ITestResult testResult) {
		...
		SeleniumContext.context = testResult.getTestContext(); // This is for
																// Cucumber
		/*
		 * ExtentTestManager.startTest(testResult.getTestClass().getName()+"."+
		 * testResult.getName().toUpperCase(),testResult.getMethod().
		 * getDescription());
		 * ExtentTestManager.getTest().assignCategory(testResult.getTestClass().
		 * getName());
		 * ExtentTestManager.getTest().log(LogStatus.INFO,"onTestStart"
		 * ,testResult.getName());
		 */
	}
	...
	public void onTestSuccess(ITestResult testResult) {
		...
		if ((ExtentManager.extent != null && AllDrive.driverSession.get() != null)
				|| testResult.getTestContext().getCurrentXmlTest().getParameter("WebDriverRequired").equals("NO")) {
	...
	public void onTestFailure(ITestResult testResult) {
		...
		if (AllDrive.driverSession.get() != null) {
			...
			try {
				...
				shotName = ScreenShot.ShotCaptured(AllDrive.getWebDriver(),
						System.getProperty("user.dir") + "/Reports/extent_reports/" + (date.getMonth() + 1) + "/"
								+ date.getDate() + "/" + testResult.getMethod().getMethodName() + date.getTime());
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (SeleniumContext.getTestLevelDriverRequired().equals("NO")) {
			ExtentTestManager.getTest().log(LogStatus.INFO, "REQ/RES As Test Failed",
					AllDrive.getWriter().toString().replaceAll("[[<>#;.(){\\/]$%%}]", " "));
			ExtentTestManager.getTest().log(LogStatus.FAIL, "Exception", "<pre><code>"
					+ testResult.getThrowable().toString().replaceAll("[[<>#;.(){\\\\/]$%%}]", " ") + "</code></pre>");
		}
		...
		if (testResult.getMethod().getRetryAnalyzer() != null) {
			...
			if (retryAnalyzer.isRetryAvailable()) {
				...
				// ExtentTestManager.getTest().addScreenCapture(shotName),testResult.getThrowable());
			}
		...
		if (AllDrive.driverSession.get() != null) {
			...
			try {
				...
				shotName = ScreenShot.ShotCaptured(AllDrive.getWebDriver(),
						System.getProperty("user.dir") + "/Reports/extent_reports/" + (date.getMonth() + 1) + "/"
								+ date.getDate() + "/" + testResult.getMethod().getMethodName() + date.getTime());
				ExtentTestManager.getTest().log(LogStatus.SKIP, ExtentTestManager.getTest().addScreenCapture(shotName),
						testResult.getThrowable());
			} catch (IOException e) {
				e.printStackTrace();
			}
	...
	public void afterInvocation(IInvokedMethod method, ITestResult testResult) {
			...
			if (!CustomAssert.map.isEmpty()) {
				if (CustomAssert.map.get(Thread.currentThread().getId()) != null
					...
					// if (testResult.getMethod().getRetryAnalyzer() != null)
					{
						...
						/*
						 * When test gets the Element not found exception or any
						 * other except assertException
						 */
						if (testResult.getStatus() == ITestResult.FAILURE) {
							...
							List<Throwable> ls = CustomAssert.verificationFailuresMap
							...
							if (size > 1) {
								for (int i = 0; i < size; i++) {
									if (ExtentManager.extent != null) {
										ExtentTestManager.getTest().log(LogStatus.WARNING, "Assert Fail :- " + i,
												ls.get(i));
									}
							...
							// Reporter.getCurrentTestResult().getMethod());
						...
						/*
						 * When test have multiple or single Custom assert
						 * failure
						 * 
						 * In that case the result of the test is by default
						 * pass but now we have to fail the test explicitly and
						 * add the failure in the report
						 * 
						 */
						if (testResult.getStatus() == ITestResult.SUCCESS) {
							// Change Pass to failure and throw custom exception
							// error
							ITestContext tc = Reporter.getCurrentTestResult().getTestContext();
							tc.getPassedTests().addResult(testResult, Reporter.getCurrentTestResult().getMethod());
							tc.getPassedTests().getAllMethods().remove(Reporter.getCurrentTestResult().getMethod());
							...
							if (retryAnalyzer.isRetryAvailable()) {
								...
								if (size > 1) {
									for (int i = 0; i < ls.size(); i++) {
										Reporter.getCurrentTestResult().setThrowable(ls.get(i));
										if (ExtentManager.extent != null) {
											ExtentTestManager.getTest().log(LogStatus.WARNING, "Assert Fail multiple",
													ls.get(i));
										}
									}
								}
								...
								if (size > 1) {
									for (int i = 0; i < size; i++) {
										if (ExtentManager.extent != null) {
											ExtentTestManager.getTest().log(LogStatus.FAIL, "Assert Fail :- " + i,
													ls.get(i));
										}
										Reporter.getCurrentTestResult().setThrowable(ls.get(i));
									}
		...
		System.out.println("After Invocation " + testResult);
	}
}